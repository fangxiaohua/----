		...
		var feedback={
				...
				jqgird_feedbackList:function(){
					...
					jQuery(feedback.grid_selector).jqGrid({
						...
						url : "/feedback/list", //这是Action的请求地址   
						//重置默认 请求参数
						prmNames:{
					...
					$("#search_btn").on("click",function(){
						$(feedback.grid_selector).jqGrid('setGridParam',{page:1,	postData:{q:$("#search").val().trim(),status:$("#feedback-status").val()}}).trigger('reloadGrid');
					});
					//回车条件查询
					...
					//角色改变重新加载jqgrid
					$("#feedback-status").on("change",function(){
						$(feedback.grid_selector).jqGrid('setGridParam',{page:1,	postData:{q:$("#search").val().trim(),status:$("#feedback-status").val()}}).trigger('reloadGrid');
					});
